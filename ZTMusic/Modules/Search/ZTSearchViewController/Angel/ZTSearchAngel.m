...
- (instancetype)initWithHostView:(__kindof UIScrollView *)hostView searchAction:(void (^)(NSString *word))searchAction
{
    if (self = [super initWithHostView:hostView]) {
        @weakify(self);
        [self setSearchAction:^(NSString *word) {
            @strongify(self);
            [self addHistorySearchWordRecord:word];
        ...
        self.addSection(ZTSearchVCSectionTypeHot);
    }
    return self;
}
...
- (void)loadHotSearchModuleWithModelArray:(NSArray *)array
{
    ...
    self.sectionForTag(sectionTag).sectionInsets(UIEdgeInsetsZero).clear();
    if (array.count > 0) {
        self.sectionForTag(sectionTag).sectionInsets(UIEdgeInsetsMake(0, 0, 50, 0));
        self.addCell(@"ZTSearchSectionTitleCell").toSection(sectionTag).withDataModel(@"热门搜索");
...
- (void)loadHistorySearchModule
{
    ...
    self.sectionForTag(sectionTag).sectionInsets(UIEdgeInsetsZero).clear();
    if (array.count > 0) {
        self.sectionForTag(sectionTag).sectionInsets(UIEdgeInsetsMake(0, 0, 15, 0));
        self.addCell(@"ZTSearchSectionTitleButtonCell").toSection(sectionTag).withDataModel(@{@"title" : @"最近搜索", @"button" : @"清空"})
